...
#This script is used to generate a morning report of any PRTG sensor
#in the down state.
...
$p = Get-Sensor | Where-Object {$_.status -eq 'Down'} #Grab all sensors from netmon in the warning state
...
foreach($u in $users){
    Send-MailMessage -to $u.mail -Subject "Netmon Report - Offline Sensors" -BodyAsHtml -body "$emailbody" -from no_reply@arcare.net -SmtpServer ds-ca.arcare.net
    }
