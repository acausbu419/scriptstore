...
for($site = 0; $1[$site] -match '\d\d\d'; $site++){
    if($1[$site] -eq $env:fa){
        for($pin = 0; $1[$pin] -match '\d\d\d'; $pin++){
            $path = Get-ChildItem -Path "\\fs-pd\facilities" | Where-Object Name -EQ $1[$pin]
...
for($site = 0; $2[$site] -match '\d\d\d'; $site++){
    if($2[$site] -eq $env:fa){
        for($pin = 0; $2[$pin] -match '\d\d\d'; $pin++){
            $path = Get-ChildItem -Path "\\fs-pd\facilities" | Where-Object Name -EQ $2[$pin]
            $qa.NameSpace($($path.FullName)).Self.InvokeVerb("pintohome")
        }
    }
    elseif($null -ne $env:fa){
...
for($site = 0; $3[$site] -match '\d\d\d'; $site++){
    if($3[$site] -eq $env:fa){
        for($pin = 0; $3[$pin] -match '\d\d\d'; $pin++){
            $path = Get-ChildItem -Path "\\fs-pd\facilities" | Where-Object Name -EQ $3[$pin]
            $qa.NameSpace($($path.FullName)).Self.InvokeVerb("pintohome")
        }
    }
    elseif($null -ne $env:fa){
...
for($site = 0; $4[$site] -match '\d\d\d'; $site++){
    if($4[$site] -eq $env:fa){
        for($pin = 0; $4[$pin] -match '\d\d\d'; $pin++){
            $path = Get-ChildItem -Path "\\fs-pd\facilities" | Where-Object Name -EQ $4[$pin]
            $qa.NameSpace($($path.FullName)).Self.InvokeVerb("pintohome")
        }
    }
    elseif($null -ne $env:fa){
...
for($site = 0; $5[$site] -match '\d\d\d'; $site++){
    if($5[$site] -eq $env:fa){
        for($pin = 0; $5[$pin] -match '\d\d\d'; $pin++){
            $path = Get-ChildItem -Path "\\fs-pd\facilities" | Where-Object Name -EQ $5[$pin]
            $qa.NameSpace($($path.FullName)).Self.InvokeVerb("pintohome")
        }
    }
    elseif($null -ne $env:fa){
...
for($site = 0; $6[$site] -match '\d\d\d'; $site++){
    if($6[$site] -eq $env:fa){
        for($pin = 0; $6[$pin] -match '\d\d\d'; $pin++){
            $path = Get-ChildItem -Path "\\fs-pd\facilities" | Where-Object Name -EQ $6[$pin]
            $qa.NameSpace($($path.FullName)).Self.InvokeVerb("pintohome")
        }
    }
    elseif($null -ne $env:fa){
...
for($site = 0; $7[$site] -match '\d\d\d'; $site++){
    if($7[$site] -eq $env:fa){
        for($pin = 0; $7[$pin] -match '\d\d\d'; $pin++){
            $path = Get-ChildItem -Path "\\fs-pd\facilities" | Where-Object Name -EQ $7[$pin]
            $qa.NameSpace($($path.FullName)).Self.InvokeVerb("pintohome")
        }
    }
    elseif($null -ne $env:fa){
...
for($site = 0; $8[$site] -match '\d\d\d'; $site++){
    if($8[$site] -eq $env:fa){
        for($pin = 0; $8[$pin] -match '\d\d\d'; $pin++){
            $path = Get-ChildItem -Path "\\fs-pd\facilities" | Where-Object Name -EQ $8[$pin]
            $qa.NameSpace($($path.FullName)).Self.InvokeVerb("pintohome")
        }
    }
    elseif($null -ne $env:fa){
...
for($site = 0; $9[$site] -match '\d\d\d'; $site++){
    if($9[$site] -eq $env:fa){
        for($pin = 0; $9[$pin] -match '\d\d\d'; $pin++){
            $path = Get-ChildItem -Path "\\fs-pd\facilities" | Where-Object Name -EQ $9[$pin]
            $qa.NameSpace($($path.FullName)).Self.InvokeVerb("pintohome")
        }
    }
    elseif($null -ne $env:fa){
...
for($site = 0; $10[$site] -match '\d\d\d'; $site++){
    if($10[$site] -eq $env:fa){
        for($pin = 0; $10[$pin] -match '\d\d\d'; $pin++){
            $path = Get-ChildItem -Path "\\fs-pd\facilities" | Where-Object Name -EQ $10[$pin]
            $qa.NameSpace($($path.FullName)).Self.InvokeVerb("pintohome")
        }
    }
    elseif($null -ne $env:fa){
