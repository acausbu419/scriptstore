...
$compname = $sysinfo.Name
$EmailReport = $true
$FileReport = $false
$To = "alerts-serverupdates@arcare.onmicrosoft.com"
$From = "no_reply@arcare.net"
$SMTPServer = "ds-ca.arcare.net"
$FileReportPath = "\\ds-msflow\c$\packer"
$AutoRestart = $true
...
$status = $pendingboot[$AutoRestartIfPending]
 $messageParameters = @{                        
                Subject = "Windows Update report for $fqdn - $((Get-Date).ToShortDateString())"                        
                Body = "Invoke-WindowsUpdate was run on $fqdn, and the server $status `nPlease run Invoke-WindowsUpdate again when the server is rebooted."               
                from = $From                        
                To = $To                      
