#ARCARE_CNC_AR_LAB.ps1
#Adam Ausburn

#This script converts a spreadsheet (using the ImportExcel module)
#into a flatfile for Centene.
#Production files are stored on fs-pd\additional\centene

#Change execution policy if needed (not signed)
#Set-ExecutionPolicy bypass -Scope Process -Force

#Script uses the ImportExcel module that can be installed
#using the below line if needed.
#Install-Module ImportExcel

Import-Module ImportExcel

#Remove old files generated by this script
Remove-Item * -Include "*.txt" -Exclude *Service*

#Naming conventions
#Centene naming convention:
#ARCARE_CNC_AR_SERVICE_$date
#ARCARE_CNC_AR_LAB_$date

#Wellcare naming convention:
#ARCARE_ESD_AR_SERVICE_$date
#ARCARE_ESD_AR_LAB_$date

#File name generation using above format
$date = Get-Date
$date = $date.ToString('yyyyMMdd')
#$file = "ARCARE_CNC_AR_LAB_$date.txt","ARCARE_ESD_AR_LAB_$date.txt"
#$file = "ARCARE_CNC_AR_LAB_.txt"
$file = "ARCARE_CNC_AR_LAB_$date.txt"

#Import the reference spreadsheet for conversion
$import = Import-Excel Lab_SDS_Template.xlsx -EndRow 200 |ConvertTo-Csv -NoTypeInformation -Delimiter '|' | % {$_.Replace('"','')} | % {$_.Replace('|||||||||||||||||||||||||||||','')}

#Remove time from date fields if present
$import = $import -replace ' 12:00:00 AM',''

#Output files
Add-content $file -Value $import